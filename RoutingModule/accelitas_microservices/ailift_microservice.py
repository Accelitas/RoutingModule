...
class AILiftMicroservice:
    ...
    def get_latest_time_stamp(cls):
        ...
        with pyodbc.connect(cls.connection_string, attrs_before = {cls.SQL_COPT_SS_ACCESS_TOKEN:cls.tokenstruct}) as connect:
            ...
            with connect.cursor() as cursor:
                ...
                cursor.execute(
                               """
                                  SELECT MAX(log_timestamp) as latest_time_stamp FROM dbo.lift_splunk;
                               """
                               )
    ...
    def get_splunk_logs_through_rest_api(cls):
        ...
        search_query = (
                        ...
                        f'|spath input=ailift_data path="AUDIT RECORD: AI-LIFT.firstPartyServiceProviderMetaData.verifyBDResponseTime" output=bd_time'
