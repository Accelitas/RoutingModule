...
class TelecheckAVMicroservice:
    ...
    def get_latest_time_stamp(cls):
        ...
        with pyodbc.connect(cls.connection_string, attrs_before = {cls.SQL_COPT_SS_ACCESS_TOKEN:cls.tokenstruct}) as connect:
            ...
            with connect.cursor() as cursor:
                ...
                cursor.execute(
                               """
                                  SELECT MAX(log_timestamp) as latest_time_stamp FROM dbo.datawire_splunk;
                               """
                               )
    ...
    def get_splunk_logs_through_rest_api(cls):
        ...
        rex = "(?P<telecheck_av_data>{.*})"

        search_query = (
                        f'search index="accelitas_k8s_cus_prod" OR index = "accelitas_k8s_eus2_prod" sourcetype="kube:container:telecheck-av"'
